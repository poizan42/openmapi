// openmapi.org - NMapi C# IMAP Gateway - CmdLogin.cs
//
...
namespace NMapi.Gateways.IMAP {
	...
	public sealed class CmdLogin : AbstractBaseCommandProcessor
	{
		...
		public CmdLogin (IMAPConnectionState state) : base (state)
		{
		}
		...
		public override void Run (Command command)
		{
			try {
				...
				state.InitServerConnection(config.Mapiserveraddress + ":" + config.MapiServerport, command.Userid, command.Password);
...
//				state.InitServerConnection("192.168.10.235", command.Userid, command.Password);
