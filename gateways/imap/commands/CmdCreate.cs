	...
	public sealed class CmdCreate : AbstractBaseCommandProcessor
	{
		...
		public override void Run (Command command)
		{
			...
			try {
				...
				string pathNew = "";
				folder = ServCon.FolderHelper.OpenFolder (string.Empty + PathHelper.PathSeparator);
				for (int i = 0; i < pathEls.Length; i++) {
					pathNew = pathNew + PathHelper.PathSeparator + pathEls[i];
					using (folderNew = ServCon.FolderHelper.OpenFolder (pathNew)) {
						if (folderNew == null) {
							Log ("CmdCreate folderName: " + pathEls[i]);
							folderNew = folder.CreateFolder (FolderType.Generic, pathEls[i], "generated by openmapi IMAP-Gateway", null, Mapi.Unicode);
				...
				if (folderNew != null) folderNew.Dispose ();
			}
		}
