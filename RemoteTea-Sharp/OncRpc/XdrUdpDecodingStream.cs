...
namespace RemoteTea.OncRpc
{
	...
	public class XdrUdpDecodingStream: XdrDecodingStream
	{
	     ...
	     *   datagrams.
	     */
	    public XdrUdpDecodingStream(UdpClient client,
	                                int bufferSize) {
	        this.client = client;
	        //
	    ...
	    public override void BeginDecoding()
	    {
	    	...
	    	senderAddress = RemoteIpEndPoint.Address;
	    	senderPort = RemoteIpEndPoint.Port;
	    	Array.Copy(receiveBytes, buffer, Math.Min(receiveBytes.Length, buffer.Length));
	        ...
	        bufferIndex = 0;
	        bufferHighmark = receiveBytes.Length - 4;
	    }
	    ...
	    public override byte [] XdrDecodeOpaque(int length)
	    {
	        ...
	        if ( length > 0 ) {
	            if ( bufferIndex <= bufferHighmark - alignedLength + 4 ) {
	        		Array.Copy(buffer, bufferIndex, bytes, 0, length);
	            } else {
	                 throw(new OncRpcException(OncRpcException.RPC_BUFFERUNDERFLOW));
	            }
	        }
