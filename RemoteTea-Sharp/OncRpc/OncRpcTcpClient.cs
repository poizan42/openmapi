...
namespace RemoteTea.OncRpc
{
	...
	public class OncRpcTcpClient: OncRpcClient
	{
	     ...
	     * @throws IOException if an I/O error occurs.
	     */
	    public OncRpcTcpClient(IPAddress host,
	    ...
	    public OncRpcTcpClient(IPAddress host,
	                           ...
	                           int bufferSize,
	                           int timeout):
	    	base(host, program, version, port, OncRpcProtocols.ONCRPC_TCP)
	    {
	        //
		...
		lock (typeof(OncRpcTcpClient)) {
			...
			while (refreshesLeft-- >= 0) {
				...
				if (transmissionTimeout < 0)		//TODO: hack
			        ...
			        throw(new OncRpcException(OncRpcException.RPC_CANTSEND, e.Message));
			    }
