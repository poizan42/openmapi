...
namespace RemoteTea.OncRpc
{
	...
	public class OncRpcUdpClient: OncRpcClient
	{
	     ...
	     */
	    public OncRpcUdpClient(IPAddress host,
	                           int program, int version,
	                           int port): this(host, program, version, port, 8192)
	    {
	    }
	    ...
	    public OncRpcUdpClient(IPAddress host,
	                           int program, int version,
	                           int port,
	                           int bufferSize): base(host, program, version, port, OncRpcProtocols.ONCRPC_UDP)
	    {
	        //
...
//	        }
	        ...
	        client = new UdpClient (); // new IPEndPoint(host, port));
	        ...
	        // communicate at all.
	        //
	        sendingXdr = new XdrUdpEncodingStream(client, bufferSize);
	        receivingXdr = new XdrUdpDecodingStream(client, bufferSize);
	    }
		...
		lock (typeof(OncRpcTcpClient)) {
			...
			while (refreshesLeft-- >= 0) 
			{
			    ...
			    long stopTime = System.DateTime.Now.AddMilliseconds(timeout).ToBinary();
			    ...
			    do {
			        ...
			        for ( ;; ) {
			            try {
			        		int currentTimeout = (int)(stopTime - System.DateTime.Now.ToBinary());
			                if ( currentTimeout > resendTimeout ) {
			                	...
			                	// infinite (?ç$@%&!!!) we need to ensure that we
			                	// have a finite timeout, albeit maybe an infinitesimal
			                	// finite one.
			                    currentTimeout = 1;
			                }
			                
			        		// TODO socket.setSoTimeout(currentTimeout);
			    ...
			    } while ( System.DateTime.Now.ToBinary() < stopTime );
			    //
		...
		public override string CharacterEncoding {
			get {
				return receivingXdr.CharacterEncoding;
			}
			set {
	     ...
	     */
	    protected int retransmissionTimeout = 0; // TODO this.timeout;
