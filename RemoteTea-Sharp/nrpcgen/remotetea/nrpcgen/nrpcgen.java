 ...
 *
 * Adapted to C# by Johannes Roith.
 *
...
import remotetea.nrpcgen.cup_runtime.Symbol;
 ...
 */
public class nrpcgen {

    /**
     * Current version of nrpcgen.
     */
    ...
    /**
     * String containing date/time when a nrpcgen run was started. This string
    ...
    /**
     * Holds information about the remote program defined in the nrpcgen
     * x-file.
     */
    ...
    /**
     * Verbosity flag. If <code>true</code>, then nrpcgen will report about
    ...
    /**
     * The name of the x-file to parse (not: the X Files, the latter ones are something
     ...
     * completely different).
     */
    public static String xFileName = null;							//TODO: This is only required for the C#-Driver to pass the information; REMOVE when ported to C#!
    ...
    /**
     * Destination directory name where to place the generated files.
     */
    public static String destinationDirName = null;						//TODO: This is only required for the C#-Driver to pass the information; REMOVE when ported to C#!

    /**
     * TypeMap XML-File
     */
    public static String typeMapFileName = null;

    /**
     * TypeMap
     */
    public static NRpcTypeMap typeMap = null;
    ...
    /**
     * Enable generation of partial classes in order make it possible to supply custom code.
     */
    public static boolean makePartialClasses = false;
    ...
    public static PrintWriter createJavaSourceFile(String classname, boolean emitImports) {
        ...
        //
        out.println("//");
        out.println("// Automatically generated by nrpcgen " + VERSION
                    + " on " + startDate);
        out.println("// nrpcgen is based on jrpcgen which is part of the \"Remote Tea\" ONC/RPC package for Java");
		...
		if ( emitImports ) {
			...
			out.println();
			out.println("using RemoteTea.OncRpc;");
			out.println();
		}
	...
	if (makePartialClasses)
	        out.print("partial ");		
        out.println("class " + baseClassname + " {");
    ...
    public static void dumpEnum(JrpcgenEnum e) {
        ...
        // the XDR enumeration.
        //
        PrintWriter out = createJavaSourceFile(typeMap.map (e.identifier), false);
        //
        // Spit out some description for javadoc & friends...
        //
        out.println("/**");
	...
	if (makePartialClasses)
	        out.print("partial ");	
        out.println("class " + typeMap.map (e.identifier) + " {");
        out.println();
        ...
        if ( data != null ) {
            ...
            if ( decl.kind == JrpcgenDeclaration.SCALAR ) {
                ...
                if ( encode ) {
                    ...
                    code.append(oref + decl.identifier);
                    code.append(" = new ");
                    code.append(typeMap.map (decl.type));
                    code.append("(xdr);\n");
                }
                return code.toString();
            //
            ...
            if ( encode ) {
                ...
                if ( decl.kind == JrpcgenDeclaration.DYNAMICVECTOR ) {
                    ...
                    //
                    code.append("xdr.XdrEncodeInt(__size); ");
                }
                //
                // Now encode all elements.
                //
                code.append("for ( int __idx = 0; __idx < __size; ++__idx ) { ");
                code.append(oref + decl.identifier);
                code.append("[__idx].XdrEncode(xdr); ");
                code.append("} }\n");
            } else {
                code.append("        { ");
                code.append("int __size = ");
    ...
    public static String checkForEnumValue(String value) {
        if ( value.length() > 0 ) {
            ...
            //
            Object id = nrpcgen.globalIdentifiers.get(value);
            if ( (id != null)
    ...
    public static void dumpStruct(JrpcgenStruct s) {
        ...
        PrintWriter out = createJavaSourceFile (typeMap.map (s.identifier));

	out.print("public ");
	if (makePartialClasses)
	        out.print("partial ");	
        out.print("class " + typeMap.map (s.identifier) + " : XdrAble");
        if ( makeSerializable ) {
        ...
        while ( decls.hasMoreElements() ) {
            ...
            out.print(access + typeMap.map (checkForSpecials(d.type)) + " ");
        ...
        if ( makeSerializable ) {
            out.println();
            out.println("    private static readonly long serialVersionUID = "
                        + hash.getHash() + "L;");
            ...
            if ( makeBean ) {
            	...
            	while ( decls.hasMoreElements() ) {
            		...
            		//
            		if ( isArray ) {
            			out.println("    public " + typeMap.map (checkForSpecials(d.type)) + "[] get" + jbName + "() { return this." + d.identifier + "; }");
            			out.println("    public " + typeMap.map (checkForSpecials(d.type)) + " get" + jbName + "(int index) { return this." + d.identifier + "[index]; }");
            		} else {
            			out.println("    public " + typeMap.map (checkForSpecials(d.type)) + " get" + jbName + "() { return this." + d.identifier + "; }");
            		}
            	}
        ...
        //
        out.println();
        out.println("    public " + typeMap.map (s.identifier) + "() {");
        out.println("    }");
        out.println();
        out.println("    /// exceptions: OncRpcException, IOException");
        out.println("    public " + typeMap.map (s.identifier) + "(XdrDecodingStream xdr) {");
        ...
        decls = s.elements.elements();
        if ( useIteration ) {
            out.println("        " + typeMap.map (s.identifier) + " __this = this;");
            out.println("        do {");
            ...
            for ( int size = s.elements.size(); size > 1; --size ) {
                ...
                out.print("    " + codingMethod(decl, true, "__this"));
            }
            decl = (JrpcgenDeclaration) decls.nextElement();
            out.println("            __this = __this." + decl.identifier + ";");
            out.println("            xdr.XdrEncodeBoolean(__this != null);");
            out.println("        } while ( __this != null );");
        } else {
            while ( decls.hasMoreElements() ) {
        ...
        if ( useIteration ) {
            out.println("        " + typeMap.map (s.identifier) + " __this = this;");
            out.println("        " + typeMap.map (s.identifier) + " __next;");
            out.println("        do {");
            JrpcgenDeclaration decl = null;
            //
            // when using the iteration loop for serializing emit code for
            ...
            for ( int size = s.elements.size(); size > 1; --size ) {
                decl = (JrpcgenDeclaration) decls.nextElement();
                out.print("    " + codingMethod(decl, false, "__this"));
            }
            decl = (JrpcgenDeclaration) decls.nextElement();
            out.println("            __next = xdr.XdrDecodeBoolean() ? new " + typeMap.map (s.identifier) + "() : null;");
            out.println("            __this." + decl.identifier + " = __next;");
            out.println("            __this = __next;");
            out.println("        } while ( __this != null );");
        } else {
            while ( decls.hasMoreElements() ) {
    ...
    public static void dumpUnion(JrpcgenUnion u) {
        ...
        // the XDR union.
        //
        PrintWriter out = createJavaSourceFile(typeMap.map (u.identifier));
	out.print("public ");
	if (makePartialClasses)
	        out.print("partial ");	
        out.print("class " + typeMap.map (u.identifier) + " : XdrAble");
        if ( makeSerializable ) {
            out.print(", java.io.Serializable");
        }
        out.println(" {");
        ...
        while ( arms.hasMoreElements() ) {
            ...
            out.print("    public " + typeMap.map (checkForSpecials(a.element.type)) + " ");
        ...
        //
        out.println();
        out.println("    public " + typeMap.map (u.identifier) + "() {");
        out.println("    }");
        out.println();
        out.println("    /// exceptions: OncRpcException, IOException {");
        out.println("    public " + typeMap.map (u.identifier) + "(XdrDecodingStream xdr) {");
        out.println("        XdrDecode(xdr);");
        out.println("    }");
        out.println();
        out.println("    /// exceptions: OncRpcException, IOException {");
    ...
    public static void dumpTypedef(JrpcgenDeclaration d) {
        ...
        // the XDR struct.
        //
        PrintWriter out = createJavaSourceFile(typeMap.map (d.identifier));
	out.print("public ");
	if (makePartialClasses)
	        out.print("partial ");	
        out.print("class " + typeMap.map (d.identifier) + " : XdrAble");
        if ( makeSerializable ) {
            out.print(", java.io.Serializable");
        }
        out.println(" {");
        ...
        out.print("    public " + typeMap.map (paramType) + " value;");
        out.println();
        ...
        out.println();
        out.println("    public " + typeMap.map (d.identifier) + "() {");
        out.println("    }");
        out.println();
        out.println("    public " + typeMap.map (d.identifier) + "(" + typeMap.map (paramType) + " value) {");
        out.println("        this.value = value;");
        out.println("    }");
        out.println();
        out.println("    /// exceptions: OncRpcException, IOException {");
        out.println("    public " + typeMap.map (d.identifier) + "(XdrDecodingStream xdr) {");
        out.println("        XdrDecode(xdr);");
        out.println("    }");
        out.println();
        out.println("    /// exceptions: OncRpcException, IOException {");
    ...
    public static void dumpClientStubMethods(PrintWriter out,
        ...
        for ( int idx = 0; idx < size; ++idx ) {
            StringBuilder innerClassExtractor = new StringBuilder ();			//TODO
            ...
            if ( proc.parameters != null ) {
                ...
                while ( params.hasMoreElements() ) {
                    ...
                    out.println("     * @param " + param.parameterName +
                                " parameter (of type " + typeMap.map (param.parameterType) +
...
///            out.println("           throws OncRpcException, IOException {");
            ...
            switch ( paramsKind ) {
            case PARAMS_VOID:
                xdrParamsName = "args__";
                out.println("        XdrVoid args__ = XdrVoid.XDR_VOID;");
                break;
            ...
            case PARAMS_SINGLE: {
                ...
                //
                // We do not need to emit an args__ declaration here, as we
            ...
            case PARAMS_SINGLE_BASETYPE: {
                ...
                out.println("        " + xdrParamsType + " args__ = new "
                            + xdrParamsType + "(" + paramInfo.parameterName + ");");
                break;
            }
            case PARAMS_MORE:
                xdrParamsName = "args__";
                innerClassExtractor.append ("    private class XdrAble__" + proc.procedureId + " : XdrAble {\n");

                int psize = proc.parameters.size();
                for ( int pidx = 0; pidx < psize; ++pidx ) {
                    JrpcgenParamInfo pinfo = (JrpcgenParamInfo)
                        proc.parameters.elementAt(pidx);
                    innerClassExtractor.append("        public "
                                + typeMap.map (checkForSpecials(pinfo.parameterType))
                                + " " + pinfo.parameterName + ";\n");
                }

                innerClassExtractor.append("        /// exceptions: OncRpcException, IOException\n");
                innerClassExtractor.append("        public void XdrEncode(XdrEncodingStream xdr) {\n");
                ...
                for ( int pidx = 0; pidx < psize; ++pidx ) {
                    ...
                    innerClassExtractor.append("    ");
                    innerClassExtractor.append(codingMethod(decl, true));
                }

                innerClassExtractor.append("        }\n");
                innerClassExtractor.append("        /// exceptions: OncRpcException, IOException\n");
                innerClassExtractor.append("        public void XdrDecode(XdrDecodingStream xdr) {\n");
                innerClassExtractor.append("        }\n");

                innerClassExtractor.append ("    };\n");

                out.println("        XdrAble__" + proc.procedureId + " args__ = new XdrAble__" + proc.procedureId + "();");
                for ( int pidx = 0; pidx < psize; ++pidx ) {
                    JrpcgenParamInfo pinfo = (JrpcgenParamInfo)
                        proc.parameters.elementAt(pidx);
                    out.println("        args__." + pinfo.parameterName + " = "
                                + pinfo.parameterName + ";");
                }
                break;
            }
            ...
            if ( resultType.equals("void") ) {
                out.println("        XdrVoid result__ = XdrVoid.XDR_VOID;");
            } else if ( xdrResultType != null ) {
                out.println("        " + xdrResultType + " result__ = new "
                            + xdrResultType + "();");
            } else {
                out.println("        " + typeMap.map (resultType) + " result__ = new "
                            + typeMap.map (resultType) + "();");
            }
            //
            ...
            if ( xdrResultType != null ) {
                ...
                //
                if ( !resultType.equals("void") ) {
                    out.println("        return result__."
                    + Character.toUpperCase (resultType.charAt(0)) + 
			resultType.substring(1).toLowerCase() + "Value();");
                }
            } else {
                //
            ...
            out.println("    }");
            out.println();
            out.println(innerClassExtractor.toString ());
        }
    }
    ...
    public static void dumpClient(JrpcgenProgramInfo programInfo) {
        ...
        //
        String clientClass = nrpcgen.clientClass;
        if ( clientClass == null ) {
        ...
        out.println(" */");
        out.println("public class " + clientClass + " : OncRpcClientStub {");
        out.println();
        //
        ...
        out.println("     * @throws IOException if an I/O error occurs.");
        out.println("     */");
        out.println("    public " + clientClass + "(IPAddress host, int protocol) : ");
        out.println("        base (host, "
        ...
        out.println("     * @throws IOException if an I/O error occurs.");
        out.println("     */");
        out.println("    public " + clientClass + "(IPAddress host, int port, int protocol) : ");
        out.println("        base (host, "
                    + baseClassname + "." + programInfo.programId + ", "
        ...
        out.println("     * @throws IOException if an I/O error occurs.");
        out.println("     */");
        out.println("    public " + clientClass + "(IPAddress host, int program, int version, int protocol) : ");
        out.println("        base (host, program, version, 0, protocol)");
        out.println("    {}");
        out.println();
        ...
        out.println("     * @throws IOException if an I/O error occurs.");
        out.println("     */");
        out.println("    public " + clientClass + "(IPAddress host, int program, int version, int port, int protocol) : ");
        out.println("        base (host, program, version, port, protocol)");
        out.println("    {}");
        out.println();
    ...
    public static void dumpServerStubMethodCall(PrintWriter out,
                                                JrpcgenProcedureInfo proc, StringBuilder innerClassExtractor) {
        //
        ...
        case PARAMS_SINGLE: {
            ...
            out.println("                call.RetrieveCall (args__);");
            params = (withCallInfo ? "call, " : "")
                     + "args__";
            break;
        }
        ...
        case PARAMS_SINGLE_BASETYPE: {
            ...
            out.println("                call.RetrieveCall (args__);");
            params = (withCallInfo ? "call, " : "")
                     + "args__." + paramsType.toLowerCase() + "Value()";
            break;
        }
        ...
        case PARAMS_MORE: {
            ...
            String innerClassExtractorNumber = checkForEnumValue(proc.procedureNumber);
            innerClassExtractor.append("                class XdrAble__" + innerClassExtractorNumber + " : XdrAble {\n");
...
////            out.println("                /// exceptions: OncRpcException, IOException {");
            ...
            for ( int pidx = 0; pidx < psize; ++pidx ) {
                ...
                decl.type = pinfo.parameterType;
                innerClassExtractor.append("                ");
                innerClassExtractor.append(codingMethod(decl, false));
            }

            innerClassExtractor.append("                    }\n");

            innerClassExtractor.append ("                };\n");
            out.println("                XdrAble__" + innerClassExtractorNumber + " args__ = new XdrAble__" + innerClassExtractorNumber + "();");
            out.println("                call.RetrieveCall (args__);");
        ...
        if ( resultType.equals("void") ) {
            ...
            out.println("                call.Reply (result__);");
        } else {
            //
            // The return type is a complex type which supports XdrAble.
            //
            out.println("                " + resultType + " result__ = "
                        + proc.procedureId + "(" + params + ");");
            out.println("                call.Reply (result__);");
        }
    }
    ...
    public static void dumpServerStubMethods(PrintWriter out,
        ...
        for ( int idx = 0; idx < procSize; ++idx ) {
            ...
            //
            out.print("    public abstract " + typeMap.map (resultType) + " "
                      + proc.procedureId + "(");
            if ( proc.parameters != null ) {
                if ( withCallInfo ) {
                    out.print("OncRpcCallInformation call__, ");
                }
                int psize = proc.parameters.size();
                for ( int pidx = 0; pidx < psize; ++pidx ) {
                    JrpcgenParamInfo paramInfo = (JrpcgenParamInfo)
                    ...
                    out.print(typeMap.map (checkForSpecials(paramInfo.parameterType)));
                    out.print(" ");
                    out.print(paramInfo.parameterName);
                }
    ...
    public static void dumpServer(JrpcgenProgramInfo programInfo) {
        ...
        // null clientClass name).
        //
        String serverClass = nrpcgen.serverClass;
        if ( serverClass == null ) {
        ...
        out.println("using System.Net;");
        out.println();
        out.println("using RemoteTea.OncRpc.Server;");
        out.println();
        ...
        //
        out.println("    /// exceptions: OncRpcException, IOException");
        out.println("    public " + serverClass + "() : this (0) ");
        out.println("    {");
        out.println("    }");
        out.println();

        out.println("    /// exceptions: OncRpcException, IOException");
        out.println("    public " + serverClass + "(int port) : this (null, port)");
        out.println("    {");
        out.println("    }");
        out.println();

        out.println("    /// exceptions: OncRpcException, IOException");
        out.println("    public " + serverClass + "(IPAddress bindAddr, int port) {");
        //
        ...
        StringBuilder innerClassExtractor = new StringBuilder ();
        ...
        for ( int versionIdx = 0; versionIdx < versionSize; ++versionIdx ) {
            ...
            for ( int procIdx = 0; procIdx < procSize; ++procIdx ) {
                ...
                dumpServerStubMethodCall(out, procInfo, innerClassExtractor);
                out.println("                break;");
                out.println("            }");
            }
            out.println("            default:");
    ...
    public static void doParse()
        ...
        destinationDir = new File (destinationDirName);
        typeMap = new NRpcTypeMap (typeMapFileName);
        ...
        try {
            ...
            throw(new FileNotFoundException("nrpcgen: can not open source x-file \""
                                            + xFile.getCanonicalPath() + "\""));
        }
        JrpcgenScanner scanner = new JrpcgenScanner(in);
        JrpcgenParser parser = new JrpcgenParser(scanner);

        nrpcgen.globalIdentifiers.put("TRUE", new JrpcgenConst("TRUE", "true"));
        nrpcgen.globalIdentifiers.put("FALSE", new JrpcgenConst("FALSE", "false"));
